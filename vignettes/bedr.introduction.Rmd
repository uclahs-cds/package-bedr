...
### Statisically Quantify Regions' Similarity (jaccard and reldist)

When comparing (pairwise) the extent of overlap between a large collection of genomic regions, it becomes necessary to report the similarity (intersect) using a single quantitative measure. Bedtools implements two such statitics; `jaccard` and `reldist` ([Favorov A et al.]([docs](http://www.ncbi.nlm.nih.gov/pubmed/22693437))) which can be called through `bedr` as shown below:
