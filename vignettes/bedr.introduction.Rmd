...
# sort - explicit calling using primary API function bedr()
b.sort <- bedr(
	engine = "bedtools", 
	input = list(i = b), 
	method = "sort", 
...
Similarly, merging adjacent or overlapping regions is often required to avoid redundancy.  Performing an intersection/join with when you have redundant regions can cause unexpected results and hence best is to merge redundant regions.
...
# merge - explicit calling using primary API function bedr()
b.merge <- bedr(
	engine = "bedtools", 
	input = list(i = b.sort), 
	method = "merge", 
...
```{r, results = "hide", message = FALSE, eval = FALSE}
# subtract
a.sub2 <- bedr.subtract.region(a.merge, b.merge);

# subtract - explicit calling using primary API function bedr()
a.sub1 <- bedr(
...
a.mult <- bedr.join.multiple.region(x = list(a.merge,b.merge,bedr.sort.region(d)));
```
