...
grow.region <- function(x, n.add = NULL, start.add = NULL, end.add = NULL, species = "human", build = "hg19", check.zero.based = TRUE, check.chr = TRUE, check.valid = TRUE, check.sort = TRUE, check.merge = TRUE, verbose = TRUE){
	...
	x <- bedr(engine = "bedtools", input = list(i = x), method = "slop", params = paste(n.add, start.add, end.add, genome.file ), check.zero.based = check.zero.based, check.chr = check.chr, check.valid = check.valid, check.sort = check.sort, check.merge = check.merge, verbose = TRUE);
