...
#	is.valid.region(y, check.zero.based = check.zero.based, check.chr = check.chr, verbose = verbose);
	...
	#	backspace <- rep("\b", nchar(i));
		...
		## FIX PLEASE
		perm.results <- mclapply(
			list(1:n.batches),
			iterate.perm,
			x.region = x,
			y.region = y,
			batch.size = batch.size,
			jaccard.orig = jaccard.orig,
			reldist.orig.median = reldist.orig.median,
			reldist.orig.fraction.zero = reldist.orig.fraction.zero,
			reldist.orig.fraction.fifty = reldist.orig.fraction.fifty,
			stratify.by.chr = stratify.by.chr,
			species = species,
			build = build,
			mask.gaps = mask.gaps,
			mask.repeats = mask.repeats, 
			gaps.file = gaps.file,
			repeats.file = repeats.file
			);

		## ERROR
		# Segmentation fault
		# Error in bedr(engine = "bedtools", input = list(a = x, b = y), method = "reldist",  :
		# In addition: Warning message:
		# In system(command, wait = TRUE, intern = intern, ignore.stdout = FALSE,  :
		#   running command 'bedtools reldist -a /tmp/RtmpxT5wu9/a_1fb74ae24ca7.bed -b /tmp/RtmpxT5wu9/b_1fb744111522.bed ' had status 139
