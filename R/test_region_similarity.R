test_region_similarity <- function (x, y, n = 1e3, stratify.by.chr = FALSE, species = "human", build = "hg19", mask.gaps = FALSE, mask.repeats = FALSE, check.zero.based = TRUE, check.chr = TRUE, check.valid = TRUE, verbose = TRUE) {

#	is_valid_region(x, check.zero.based = check.zero.based, check.chr = check.chr, verbose = verbose);
#	is_valid_region(y, check.zero.based = check.zero.based, check.chr = check.chr, verbose = verbose);
	...
	#	backspace <- rep("\b", nchar(i));
		...
		perm.results <- mclapply(list(1:n.batches), iterate_perm, x.region = x, y.region = y, batch.size = batch.size, jaccard.orig = jaccard.orig, reldist.orig.median = reldist.orig.median, reldist.orig.fraction.zero = reldist.orig.fraction.zero, reldist.orig.fraction.fifty = reldist.orig.fraction.fifty, stratify.by.chr = stratify.by.chr, species = species, build = build, mask.gaps = mask.gaps, mask.repeats = mask.repeats);
...
#			}
	...
	return(list(results = perm.results, n = n, relative_distance = reldist.orig))
	}

# permute region and compare with original
# function used for parrallelization
iterate_perm <- function(x, x.region, y.region, batch.size, jaccard.orig, reldist.orig.median, reldist.orig.fraction.zero, reldist.orig.fraction.fifty, stratify.by.chr, species, build, mask.gaps, mask.repeats) {
	...
	for (i in 1:batch.size) {
		x.perm       <- permute_region(x.region, stratify.by.chr = stratify.by.chr, species = species, build = build, mask.gaps = mask.gaps, mask.repeats = mask.repeats, is.checked = TRUE, sort.output = TRUE);
