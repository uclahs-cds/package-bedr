plot_region <- function(input, filename = NULL, type = "venn", feature = "interval", fraction.overlap = 1e-9, group = NULL, params = list(), verbose = TRUE) {
	...
	if (type == "venn") {
		...
		if (n == 1) {
			input[[1]]   <- sort_region(input[[1]]);
			input[[2]]   <- merge_region(input[[1]], number = TRUE);
			last.var     <- names(input[[2]])[ncol(input[[2]])]
			input[[2]]   <- input[[2]][input[[2]][,last.var] >  1,];
			names(input) <- c("Original","Merged");
			main <- "Merged Intervals"
			sub <- paste0("n_before=", nrow(as.data.frame(input[[1]])), "  n_after=",nrow(as.data.frame(input[[2]])));
			n <- 2;
			}
		...
		if (feature == "gene") { # gene overlap
			...
			for (i in 2:n) {
				input.overlap[[names(input)[i]]] <- bed2index(input[[1]][in_region(input[[1]], input[[i]])]);
				}
			}
			...
			input.overlap <- join_multiple_region(input, cluster = TRUE);
			}
		else if (feature == "bp") {
			input.overlap <- join_multiple_region(input);
