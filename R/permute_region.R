permute_region <- function(x, stratify.by.chr = FALSE, species = "human", build = "hg19", mask.gaps = FALSE, mask.repeats = FALSE, sort.output = TRUE, is.checked = FALSE, check.zero.based = TRUE, check.chr = TRUE,check.valid = TRUE, verbose = TRUE) {
	...
	if (stratify.by.chr) {
		...
		chr.length <- get_chr_length(chr = chr.names);
		}

	# load the masks
	if (mask.gaps) {
		gaps <- query_ucsc(system.file("extdata/gap.txt.gz", package = "bedr"), columns.keep = c("chrom","chromStart","chromEnd"));
	...
	if (mask.repeats) {
		...
		repeats <- query_ucsc("~/bedr/data/rmsk.txt.gz", columns.keep = c("genoName", "genoStart","genoEnd"));
