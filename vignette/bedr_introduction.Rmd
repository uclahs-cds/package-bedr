...
%\VignetteIndexEntry{An Introduction to the **bedr** package}
-->
...
3. tabix [docs]() and [source](/oicr/local/analysis/sw/tabix/tabix-0.2.6)

General Region Utilities
----------------------------

* sort_region & is_sorted_region
* merge_region & is_merged_region
* in_region & order_region
* subtract_region
* flank_region & grow_region
* intersectRegion
* join_region & join_multiple_region
* get.RegionSize
* get_fastaRegion
* jaccard & reldist
...
index <- get_example_regions();
a <- index[[1]];
b <- index[[2]];

is.a.valid  <- is_valid_region(a);
is.b.valid  <- is_valid_region(b);
a <- a[is.a.valid];
b <- b[is.b.valid];

```

Generally, it's a good idea to confirm that you've sorted your inputs to avoid unexpected results and take advantage of optimized algorithms.  For example merging and clustering require adjacent intervals.  `is_sorted_region` is convenient for explicit evaluation although it's done internally for core operations.


```{r}
### sort
is.sorted <- is_sorted_region(a);

a.sort <- sort_region(a);
...
```{r}
### merge or cluster
is.merged <- is_merged_region(a.sort);
is.merged <- is_merged_region(b.sort);

a.merge <- merge_region(a.sort);
...
a.sub2 <- subtract_region(a.merge1, b.merge1);
...
```{r}
### in
is.region <- in_region(a.merge1, b.merge1);
# or alternatively
...
```{r}
### intersect / join
a.int3 <- join_region(a.merge1, b.merge1);
a.int1 <- bedr(input = list(a = a.sort1, b = b.sort1), method = "intersect", params = "-loj -sorted");

### multiple join
d <- get_random_regions(100, chr="chr1", sort = TRUE);
a.mult <- join_multiple_region(x = list(a.merge1,b.merge1,sort_region(d)));
```
...
There are several approaches to visualization of genomic ranges including venn diagrams and genome viewers.  Several examples on how to use venn diagrams include.
...
These tools are in development and will be documented at a later time. 
...
Example 1: Exome Target Processing
----------------------------
...
read_vcf to load some clinical data annotation

read_vcf to load some variants, 1kg??
